...
def process_cropped_image_bright(cropped_image):

    # Increase brightness by adjusting alpha (contrast) and beta (brightness)
    alpha = 1.5  # Simple contrast control [1.0 means no change] 0.0 to 3.0
    beta = 0  # Simple brightness control [0 means no change] -100 to 100
...
def process_and_save_frame(frame_file, word_path, output_word_path):
    ...
    # Apply processing
    processed_image = process_cropped_image_bright(cropped_image)
...
def main():
    source_dir = r"D:\PycharmProjects\pro_dis_2\collected_data\exceptions\2_augmented_datasets"
    output_dir = r"D:\PycharmProjects\pro_dis_2\collected_data\exceptions\3_processed frames"
