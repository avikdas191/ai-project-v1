...
def process_word_folder(word_folder, source_dir, output_dir):
    ...
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(process_and_save_frame, frame_file, word_path, output_word_path) for frame_file in
                   frame_files]
...
def main():
    source_dir = "D:/PycharmProjects/pro_dis_2/collected_data/cropped"
    output_dir = "D:/PycharmProjects/pro_dis_2/collected_data/processed_cropped"

    # Get list of all word folders
    word_folders = [folder for folder in os.listdir(source_dir) if os.path.isdir(os.path.join(source_dir, folder))]

    # Process each word folder sequentially
    for word_folder in word_folders:
        process_word_folder(word_folder, source_dir, output_dir)

    print("Processing completed for all word folders.")
