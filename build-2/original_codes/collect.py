...
while True:
    ...
    for face in faces:
        ...
        if (determining_lip_distance != 0 and LIP_DISTANCE_THRESHOLD != None):
            ...
            if lip_distance > LIP_DISTANCE_THRESHOLD:  # person is talking
                ...
                if not_talking_counter >= NOT_TALKING_THRESHOLD and len(
                        curr_word_frames) + PAST_BUFFER_SIZE == TOTAL_FRAMES:
                    ...
                    print(f"adding {label.upper()} shape", lip_frame.shape, "count is", data_count, "frames is",
                ...
                elif not_talking_counter < NOT_TALKING_THRESHOLD and len(
                        curr_word_frames) + PAST_BUFFER_SIZE < TOTAL_FRAMES and len(
                        curr_word_frames) > VALID_WORD_THRESHOLD:
                    cv2.putText(frame, "RECORDING WORD RIGHT NOW", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, ORANGE, 2)
                ...
                # too little frames, discard the data
                        ...
                        not_talking_counter >= NOT_TALKING_THRESHOLD and len(
                        curr_word_frames) + PAST_BUFFER_SIZE > TOTAL_FRAMES):
                    cv2.putText(frame, "NOT RECORDING WORD", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, BLUE, 2)
            ...
            cv2.putText(frame, "Current distance: " + str(distance + 2), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        (255, 255, 255), 2)
    ...
    cv2.putText(frame, "COLLECTED WORDS: " + str(len(all_words)), (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255),
                2)
...
def process_frames(all_words, labels):
    ...
    indices_to_keep = [i for i, sublist in enumerate(all_words) if
                       (len(sublist) >= median_length and len(sublist) <= median_length + 2)]
